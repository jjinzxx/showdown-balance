...
> GDD v2.5의 코어 게임플레이 규칙을 그대로 구현한 몬테카를로 시뮬레이터로, 플레이테스트 전 수치를 사전 검증합니다.  
> 라이트 모던 테마로 전환했고, LLM 보조 판단·LLM 음성 파이프라인을 근사 반영해 P2(라이벌)의 LLM 개입이 밸런스에 미치는 영향을 Baseline과 비교 확인할 수 있습니다.
...
스테이지 프리셋 버튼으로 GDD의 압박 점층 곡선 수치를 원클릭 적용합니다.  
...
| 레이즈 상한 | 가변 레이즈 최대 발수 |
| '7'+폴드 6발 강제 | 예외 규칙 on/off |
| 저카드 보정 토글 | 단조화 해법 후보 실험용 |
...
> GDD는 "'가장 낮은 카드부터 주기'가 압도적 정답이 되어 카드 선택이 기계적으로 수렴한다"고 기록했다.
...
**→ P1 제공 카드 다양성(엔트로피)은 1.00이지만 실제 선택은 최저에 100% 수렴.** 이는 GDD가 지적한 현상 그대로이며, '전술 혼합' 전략(보스 모델)을 적용해도 단조화를 완전히 막지는 못한다.
...
|---|---|---|
| 단조화 실제 문제 여부 | P1 최저카드 제공률 100% 재현 | 플레이어가 실제로 항상 최저를 주는지 관찰 |
| 폴드 동기 약함 | 폴드율 14.5% (시뮬) | 실측 폴드 빈도와 비교 |
| 게임 길이 목표 달성 | Stage 1 ≈ 9판 | 실제 플레이 소요 시간·판 수 기록 |
| 저카드 보정 효과 | 보정 ON/OFF 승률·길이 비교 | 보정 룰 추가 후 체감 단조화 변화 수집 |
| LLM 개입 영향 | LLM 보조판단 ON/OFF 승률·길이·폴드율 비교 (⑧) | 실제 채팅·음성 도발 빈도, 파이프라인 성공률과 시뮬 설정값 비교 |

<br><br>

## LLM Voice/Chat 개입 

P2(라이벌)의 기본 행동은 여전히 코드 성향 파라미터가 결정합니다. 시뮬레이터의 ③ 섹션에서 이를 켜면, 플레이어의 채팅·음성 도발이 STT→LLM→TTS 파이프라인을 거쳐 라이벌 판단에 제한된 영향을 주는 상황을 근사 모델링합니다.

- **평균 도발 강도(provocation)** · **파이프라인 성공률**(낮을수록 폴백 빈도↑) · **누적 자존심 타격(ai_mental_damage) 반영 여부**를 조절
- 개입이 발동하면 공격성·블러핑 빈도를 올리고 폴드 성향을 낮추는 방향으로만 조정 — 카드 정보·확률·목숨·승패 규칙은 건드리지 않음(안전장치를 그대로 근사)
- **⑧ LLM 영향도 비교**: 동일 시드로 Baseline(LLM OFF)과 LLM ON을 각각 돌려 승률·게임 길이·폴드율·평균 베팅 변화량을 표로 대조

이 근사 모델은 실제 LLM 호출을 재현하는 것이 아니라, GDD가 정의한 허용 범위 안에서 성향 파라미터를 조정하는 수치 시뮬레이션입니다.
